...
// variable to detect the user input via UART
volatile int uart_rx=0;
...
void InitUart(void)
{
	...
	// Calculating sbr and setting baud rate with over-sampling rate of 16
...
void UART0_IRQHandler(void)
{
	...
	if (UART0->S1 & UART0_S1_RDRF_MASK){
		uart_rx=1;									// update user input detect status
...
{
	if(uart_rx){
		uart_rx=0;						//updates the user input detect status
		return 1;
	}else
	return 0;
}
